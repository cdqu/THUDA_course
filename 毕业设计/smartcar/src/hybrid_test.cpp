...
#include "smartcar/param_reader.h"
...
{ 
    ...
    if(reader.loadParam(argv[1]))
    {
        ...
        double time_tolerance = 30, step_size = 5.0, turning_penalty_factor = 5.0;
        ...
        if(planner->plan(start, goal, solution)){
            ...
            {
                std::cout << solution[i].getX() << " " << solution[i].getY() << " " << solution[i].getTheta() << std::endl;
            }
            ...
            cv::imwrite("/home/qucd/test_ws/src/smartcar/res_pic/plan_res.jpg", img);
            cv::imshow("xxx", img);
            cv::waitKey(0);
        }else{
            auto te = std::chrono::system_clock::now();
