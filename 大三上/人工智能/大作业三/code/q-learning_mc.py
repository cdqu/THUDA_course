...
def take_eg_action(state, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        action = np.argmax(qtable[state_ind])
    return action
...
#                             size=(state_size + [env.action_space.n]))  # 随机初始化q表
qtable = np.zeros(state_size + [env.action_space.n])  # q表全零
...
for ep in range(episode):
    ...
    while not done:
        ...
        if not done:  # 未成功，使用q-learning算法更新Q表
            q_real = reward + gamma * np.max(qtable[get_state_ind(next_state)])
            qtable[get_state_ind(state)][action] += lr * (q_real - qtable[get_state_ind(state)][action])
        elif next_state[0] >= 0.5:  # 成功抵达
            qtable[get_state_ind(state)][action] = 0
...
state = env.reset()
while not done:
    action = np.argmax(qtable[get_state_ind(state)])
